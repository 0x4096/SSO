 ...
 * @date: 2019/1/6
 * @instructions: 连接Redis工具集,参考代码:https://raw.githubusercontent.com/xuxueli/xxl-sso/master/xxl-sso-core/src/main/java/com/xxl/sso/core/util/JedisUtil.java
 */
public class JedisUtils {
    ...
    private static ShardedJedis getInstance() {
        ...
        if (shardedJedisPool == null) {
            throw new NullPointerException("JedisUtils.ShardedJedisPool is null.");
        }
     ...
     * @return
     */
    public static String setObjectValue(String key, Object value) {
        return setObjectValue(key,value,-1);
    }
     ...
     * @param seconds 存活时间,单位/秒
     */
    public static String setObjectValue(String key, Object value, int seconds) {
        String result = null;
        ShardedJedis client = getInstance();
        if(seconds == -1){
            result = client.set(key.getBytes(),serialize(value));
        }else{
            result = client.setex(key.getBytes(), seconds, serialize(value));
        }
        if (client != null) {
            client.close();
        }
